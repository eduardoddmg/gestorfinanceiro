...
router.post("/createUser", async (req, res) => {
    try {
        ...
        if (!userData.username || !userData.password || !userData.email) {
            res.status(417).json({ type: 'error', message: "Missing data in body request" });
        }

        if (await userSchema.exists({ username: userData.username })) {
            res.status(417).json({ type: 'error', message: "Username already in use" });
        }

        if (await userSchema.exists({ email: userData.email })) {
            res.status(417).json({ type: 'error', message: "Email already in use" });
        }
